...
def eval_fidiou(args, sw, model_G,model_seg, data_loader,epoch=-1):
    ...
    for i, sample in enumerate(data_loader):
        ...
        imgs_plus=torch.cat((imgs,feature_map,id_layer.float(),boundary_aware_tf.float()),1)
...
def train(args, get_dataloader_func=get_pix2pix_maps_dataloader):
    ...
    G = get_G(args,input_nc=3+256+1+1) # 3+256+1，256为分割网络输出featuremap的通道数
    ...
    for epoch in range(epoch_now, args.epochs):
        ...
        for step, sample in enumerate(data_loader):
            ...
            id_layer_np = []
            im_path = sample['A_paths']
            layer_num = list(range(args.layer_num))
            layer_num_norm = [x/4 for x in layer_num]
            for i in range(len(im_path)):
                num_layer=int(im_path[i].split(os.sep)[-2])
                id_layer_np.append(np.full((1,args.fineSize,args.fineSize),layer_num_norm[num_layer-1]))
            ...
            imgs_plus = torch.cat((imgs_plus_old,id_layer.float(),boundary_aware_tf.float()),1)
