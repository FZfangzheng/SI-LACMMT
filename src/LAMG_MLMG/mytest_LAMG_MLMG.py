...
def eval_fidiou(args, sw, model_G,model_seg, data_loader,epoch=-1):
    ...
    for i, sample in enumerate(data_loader):
        ...
        imgs_plus=torch.cat((imgs, imgs,feature_map,boundary_aware_tf.float()),1)
    ...
    real_paths = [osp.join(real_dir,"1")]
    fake_paths = [osp.join(fake_dir, "1")]

    rets = eval_epoch(real_paths,fake_paths,epoch, eval_dir)
    if epoch != -1:
        sw.add_scalar('eval1/kid_mean', rets[0].kid_mean, int(epoch/20))
        sw.add_scalar('eval1/fid', rets[0].fid, int(epoch / 20))
        sw.add_scalar('eval1/kNN', rets[0].kNN, int(epoch / 20))
        sw.add_scalar('eval1/K_MMD', rets[0].K_MMD, int(epoch / 20))
        sw.add_scalar('eval1/WD', rets[0].WD, int(epoch / 20))
        sw.add_scalar('eval1/_IS', rets[0]._IS, int(epoch/20))
        sw.add_scalar('eval1/_MS', rets[0]._MS, int(epoch / 20))
        sw.add_scalar('eval1/_mse_skimage', rets[0]._mse_skimage, int(epoch / 20))
        sw.add_scalar('eval1/_ssim_skimage', rets[0]._ssim_skimage, int(epoch / 20))
        sw.add_scalar('eval1/_ssimrgb_skimage', rets[0]._ssimrgb_skimage, int(epoch / 20))
        sw.add_scalar('eval1/_psnr_skimage', rets[0]._psnr_skimage, int(epoch / 20))
        sw.add_scalar('eval1/_kid_std', rets[0]._kid_std, int(epoch / 20))
        sw.add_scalar('eval1/_fid_inkid_mean', rets[0]._fid_inkid_mean, int(epoch / 20))
        sw.add_scalar('eval1/_fid_inkid_std', rets[0]._fid_inkid_std, int(epoch / 20))
...
def train(args, get_dataloader_func=get_pix2pix_maps_dataloader):
    ...
    G = get_G(args,input_nc=3+3+256+1) # 3+256+1，256为分割网络输出featuremap的通道数
    ...
    if epoch_now==args.epochs or args.epochs==-1:
        ...
        iou = eval_fidiou(args,sw=sw,model_G=G, model_seg=DLV3P,data_loader=get_pix2pix_maps_dataloader(args, train=False),epoch=epoch_now)
    ...
    for epoch in range(epoch_now, args.epochs):
        ...
        for step, sample in enumerate(data_loader):
            ...
            #     id_layer_np.append(np.full((1,args.fineSize,args.fineSize),num_layer))
            # feature_map=feature_map.detach()
            imgs_plus_old = torch.cat((imgs,imgs,feature_map),1) # bs*(3+256)*h*w

            # print(id_layer.shape)
            imgs_plus = torch.cat((imgs_plus_old,boundary_aware_tf.float()),1)
            # train the Discriminator
            D_optimizer.zero_grad()
            # print(soft_outputs.detach().float())
        ...
        if epoch%20==0 or epoch == (args.epochs-1):
            ...
            fid,iou = eval_fidiou(args,sw=sw,model_G=G, model_seg=DLV3P,data_loader=get_pix2pix_maps_dataloader(args2, train=False),epoch=epoch)
