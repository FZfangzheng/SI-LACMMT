...
def L1_rgb_grad_loss(img0, img1):
    # 进行了改动，之前的函数太过分了，是写死的，并且只适用bs=1
    batch,channel,w,h=img0.size()
    ...
    for i in range(batch):
        img0_=img0[i]
        img1_=img1[i]
        rgb_grad_0 = cal_rgb_grad(img0_)
        rgb_grad_1 = cal_rgb_grad(img1_)
        grad_batch_0.append(rgb_grad_0.expand(batch,-1,-1, -1))
        grad_batch_1.append(rgb_grad_1.expand(batch,-1,-1,-1))
    grad_batch_0=torch.cat(grad_batch_0,0)
    grad_batch_1=torch.cat(grad_batch_1,0)
