...
def train(args, get_dataloader_func=get_pix2pix_maps_dataloader):
    ...
    for epoch in range(epoch_now, args.epochs):
        ...
        for step, sample in enumerate(data_loader):
            ...
            imgs_plus_old = torch.cat((imgs,feature_map,domain),1) # bs*(3+256)*h*w
            ...
            with torch.no_grad():
                outputs_c, feature_map_c = swin_transformer.forward_test(img_c_seg)
                imgs_plus_c = torch.cat((img_c, feature_map_c, domain_c), 1)  # bs*(3+256)*h*w
                fakes_c = G(imgs_plus_c)
            fakes_c = fakes_c.detach()

            # print('fakes.shape', fakes.shape)
            # print('fakes_c.shape', fakes_c.shape)
            fake_refined = G_consis(fakes,fakes_c)
            ...
            if epoch >= args.start_train_C_epoch:
                C_fake_cls = C(fakes.float())
                C_loss_gan_cls = classification_loss(C_fake_cls, domain_label)
            ...
            G_loss += gan_loss
            if epoch >= args.start_train_C_epoch:
                G_loss += C_loss_gan_cls
            gan_loss = gan_loss.mean().item()
            if epoch >= args.start_train_C_epoch:
                C_loss_gan_cls_item = C_loss_gan_cls.mean().item()
            ...
            if args.tensorboard_log and step % args.tensorboard_log == 0:  # defalut is 5
                ...
                if epoch >= args.start_train_C_epoch:
                    sw.add_scalar('Loss/c_gan', C_loss_gan_cls_item, total_steps)
                ...
                if epoch >= args.start_train_C_epoch:
                    sw.add_scalar('LR/C', get_lr(C_optimizer), total_steps)
        ...
        if epoch == (args.epochs-1) or (epoch%100==0 and epoch!=0):
            ...
            eval_fidiou(args,sw=sw,model_G=G, model_seg=swin_transformer,model_G_consis=G_consis,data_loader=get_pix2pix_maps_dataloader(args2, train=False,flag=14),epoch=epoch)
