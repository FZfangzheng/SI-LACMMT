...
def get_lr_poly(total_epochs=200):
    half_epochs = total_epochs // 2
    # plus one to avoid zero lr
    lr_lambda = lambda epoch: 1.0 if epoch < half_epochs \
        else float(total_epochs - epoch + 1) / float(total_epochs - half_epochs + 1)
...
def get_poly_scheduler(args, optimizer):
    scheduler = optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=get_lr_poly(args.epochs))
    return scheduler
