...
class deeplabv3plus(nn.Module):
    def __init__(self, cfg):
        ...
        #ern 所需层
        self.b_e=nn.ConvTranspose2d(64,64,3,2,1,output_padding=1)
        self.c_e=nn.Sequential(
        ...
        self.encoder_edgeconv=nn.Sequential(
            nn.Conv2d(128, 64, 3, 1, padding=1, bias=True),
            SynchronizedBatchNorm2d(64, momentum=cfg.TRAIN_BN_MOM),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, 64, 3, 1, padding=1, bias=True),
            SynchronizedBatchNorm2d(64, momentum=cfg.TRAIN_BN_MOM),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, 2, 1, 1, padding=0),
        )
        self.decoder_edgeconv = nn.Sequential(
            nn.Conv2d(128, 64, 3, 1, padding=1, bias=True),
            SynchronizedBatchNorm2d(64, momentum=cfg.TRAIN_BN_MOM),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, 64, 3, 1, padding=1, bias=True),
    ...
    def forward(self, x):
        ...
        # 以下推算两个edgemap
        b_e=self.b_e(ern_layers[0])
        c_e=self.c_e(ern_layers[1])
        f_e=self.f_e(feature_map)
        bc_e=torch.cat([b_e, c_e], 1)
        self.en_edge=self.encoder_edgeconv(bc_e)
        self.de_edge=self.decoder_edgeconv(f_e)
    ...
    def get_edges(self):
        return self.en_edge,self.de_edge
