...
class deeplabv3plus(nn.Module):
    def __init__(self, cfg):
        ...
        self.cat_conv_2 = nn.Sequential(
            nn.Conv2d(cfg.MODEL_ASPP_OUTDIM + 3, cfg.MODEL_ASPP_OUTDIM, 3, 1, padding=1,
                      bias=True),
            SynchronizedBatchNorm2d(cfg.MODEL_ASPP_OUTDIM, momentum=cfg.TRAIN_BN_MOM),
            nn.ReLU(inplace=True),
            nn.Dropout(0.5),
    ...
    def forward(self, x):
        ...
        feature_map_2=self.upsample4(feature_map)
        feature_cat_2=torch.cat([feature_map_2,x],1)
        feature_map_final=self.cat_conv_2(feature_cat_2)


        result = self.cls_conv(feature_map_final)
        return result,feature_map_final
